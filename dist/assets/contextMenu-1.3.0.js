import{n as y,p as f,t as h,q as C,O as r,j as E,k as S,r as x,u as b,C as T,b as P,v as L,w as D,x as k,E as H}from"./obrMetadataMigration-1.3.0.js";function I(e,t){return Math.max(0,e+t)}function u(e){return e.replace(/[&<>'"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[t])}function i(e){return e===void 0||e===""?"—":u(String(e))}function O(e,t){const a=e.damageDescription?.trim(),d=e.damageTags?.trim();return`
    ...
    ${t?`<div class="summary-row load-row ${f(h(t.inventory),t.fields.maxLoad)?"load-warning":""}">
          ${C(t.inventory,t.fields.maxLoad)}
          ${f(h(t.inventory),t.fields.maxLoad)?"<strong>Overloaded</strong>":""}
        </div>`:""}`}const o=document.querySelector("#context-menu"),M=new URL("./",window.location.href),R=new URLSearchParams(window.location.search),q=R.get("preview");let s,v=!1,l,n,p;function m(e){const t=e.metadata[T];return P(t)?t:{}}function w(e){const t=document.documentElement;t.dataset.obrTheme=e.mode.toLowerCase(),t.style.setProperty("--dw-text",e.text.primary),t.style.setProperty("--dw-text-secondary",e.text.secondary),t.style.setProperty("--dw-primary",e.primary.main)}function g(){if(!s){o.innerHTML='<p class="loading">Select one character token.</p>';return}const e=m(s),t=(e.moves??"").split(/\r?\n/).map(a=>a.trim()).filter(Boolean);o.innerHTML=`
    <section class="panel">
...
Call the marsh to its aid`,treasure:"A waterlogged purse and a silver idol",visibleToPlayers:!0}}},p={schemaVersion:3,id:"preview-character",fields:{name:"Frogman",maxLoad:11},inventory:[["Coin",.01,137],["Bag of Books",.4,3]],revision:1,createdAt:"2026-07-27T12:00:00.000Z",createdBy:"preview",updatedAt:"2026-07-27T12:00:00.000Z",updatedBy:"preview",writeId:"preview-write"};const e=R.get("theme")==="light";document.documentElement.dataset.obrTheme=e?"light":"dark",document.documentElement.style.setProperty("--dw-text",e?"#27272a":"#f4f4f5"),document.documentElement.style.setProperty("--dw-text-secondary",e?"#52525b":"#d4d4d8"),document.documentElement.style.setProperty("--dw-primary","#7c3aed"),g()}else r.isAvailable?r.onReady(async()=>{try{await E()}catch(e){console.error("DWTools metadata namespace migration failed",e),o.innerHTML='<p class="error">DWTools could not migrate its saved data. Reload Owlbear and try again.</p>',await r.notification.show("DWTools could not migrate its saved data.","ERROR");return}n=S(),l=x(n),w(await r.theme.getTheme()),r.theme.onChange(w),await c(),r.player.onChange(()=>{c()}),r.scene.items.onChange(e=>{if(!s)return;const t=e.find(a=>a.id===s.id);t&&(s=t,c())}),n.subscribe(e=>{const t=s&&b(s);t&&e.some(a=>a.characterId===t.characterId)&&c()})}):o.innerHTML='<p class="error">Open this menu inside Owlbear Rodeo.</p>';
