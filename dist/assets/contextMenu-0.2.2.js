import{O as a,C as c,i as h,c as g}from"./constants-0.2.2.js";import{p as w,f as T,r as R}from"./damage-0.2.2.js";import{i as E}from"./icons-0.2.2.js";const n=document.querySelector("#context-menu"),P=new URL("./",window.location.href);let s,l=!1;function y(e){return e.replace(/[&<>'"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[t])}function r(e){const t=e.metadata[c];return h(t)?t:{}}function o(e){return e===void 0||e===""?"—":y(String(e))}function u(e){const t=document.documentElement;t.dataset.obrTheme=e.mode.toLowerCase(),t.style.setProperty("--dw-text",e.text.primary),t.style.setProperty("--dw-text-secondary",e.text.secondary),t.style.setProperty("--dw-primary",e.primary.main)}function f(){if(!s){n.innerHTML='<p class="loading">Select one character token.</p>';return}const e=r(s),t=(e.moves??"").split(/\r?\n/).map(i=>i.trim()).filter(Boolean);n.innerHTML=`
    <section class="panel">
      <div class="stats">
        <button class="visibility-button" type="button" id="visibility" aria-label="${e.visibleToPlayers===!1?"Hidden from players":"Visible to players"}" title="${e.visibleToPlayers===!1?"Hidden from players":"Visible to players"}">
          <span class="dw-icon" aria-hidden="true">${E(e.visibleToPlayers===!1?"eye-off":"eye")}</span>
        </button>
